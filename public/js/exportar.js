...
// ============================================================
// EXPORTAR — Exportação de Relatórios para Excel via SheetJS
// ============================================================
  ...
  run(module, options = {}) {
    ...
    if (module === 'all' || module === 'bookings') {
      ...
      const dataToExport = bookings.map(b => {
        ...
        return {
          ...
          Pessoas: b.people || 1,
          Status: (STATUS_LABELS[b.status]?.label) || '—',
          Guia: g?.name || '—',
          Motorista: d?.name || '—',
          'Valor Total': b.totalValue || 0,
          Pago: b.paid || 0,
          Pendente: (b.totalValue || 0) - (b.paid || 0),
          Obs: b.notes || '—',
        };
      });
    ...
    if (module === 'all' || module === 'packages') {
      addSheet('Pacotes', DB.get(DB.KEYS.PACKAGES).map(p => ({
        Nome: p.name || '—',
        Destino: p.destination || '—',
        Duração: (p.duration || 0) + 'd',
        Preço: p.price || 0,
        Capacidade: p.capacity || 0,
        Status: (STATUS_LABELS[p.status]?.label) || '—',
    ...
    if (module === 'all' || module === 'cashflow') {
      ...
      addSheet('Caixa', cf.map(c => ({
        ...
        Tipo: c.type === 'income' ? 'Entrada' : 'Saída',
        Categoria: c.category || '—',
        Descrição: c.description || '—',
        Valor: c.amount || 0,
      })));
      const income  = cf.filter(c => c.type === 'income').reduce((s, c) => s + (c.amount || 0), 0);
      const expense = cf.filter(c => c.type === 'expense').reduce((s, c) => s + (c.amount || 0), 0);
    ...
    if (!wb.SheetNames || wb.SheetNames.length === 0) {
      showToast('⚠️ Nenhum dado registrado nesta categoria para exportar!', 'warning');
      return;
    }
